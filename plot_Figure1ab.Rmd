...
## Plot bar plot of explained variance per feture type
...
```{r}
#subdata <- subset (data, Space %in% c ("MGS", "KEGG pathway", "CARD ARO", "Lipoproteins", "Serum"))
...
p <- ggplot (subdata[order(subdata$Category), ], aes (x = `Feature space`, y = `Total adjusted R squared from significant factors`)) + 
  ...
  xlab ("") + ylab ("") + scale_fill_brewer (type = "qual", palette = "Paired")
